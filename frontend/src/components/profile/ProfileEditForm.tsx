  ...
  return (
    ...
    >
      <div className="space-y-6">
        <div className="space-y-4">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5 ml-1">
            <User size={12} /> {t("profile.personalDetails")}
          </label>
          <AuthInput
            name="name"
            type="text"
        ...
        <div className="space-y-4 pt-4 border-t border-slate-800/50">
          <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center gap-1.5 ml-1">
            <Lock size={12} /> {t("profile.securityLabel")}
          </label>
      ...
      <div className="flex flex-col sm:flex-row gap-3 pt-4">
        ...
        >
          {isLoading ? (
            <span className="flex items-center gap-2">{t("profile.saving")}</span>
          ) : (
            <span className="flex items-center gap-2">{t("profile.updateProfile")}</span>
          )}
        </button>
        <button
          type="button"
        ...
        >
          <RotateCcw size={18} /> {t("profile.cancel")}
        </button>
      </div>
    </form>
  );
